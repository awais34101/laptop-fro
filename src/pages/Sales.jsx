...
export default function Sales() {
  ...
  const handleGeneratePDF = (sale) => {
    const pdfData = generateInvoicePDF(sale, 'store1');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const row of completeRows) {
        ...
        if (requestedQty > availableStock) {
          inventoryErrors.push(
            `❌ ${itemName}: Cannot sell ${requestedQty} units - only ${availableStock} units available in Store 1 inventory`
          );
        }
      }
      ...
      const itemsPayload = completeRows.map(r => ({ item: r.item, quantity: Number(r.quantity), price: Number(r.price) }));
      if (editId) {
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Track and manage all sales invoices
          </Typography>
        </Box>
        {hasPerm('sales','view') && (
          <Button 
              ...
              <Table>
                ...
                <TableBody>
                  {s.items && s.items.map((item, idx) => (
                    ...
                    >
                      ...
                      <TableCell sx={{ fontWeight: 700, color: '#d32f2f' }}>
                        {(item.quantity && item.price) ? (Number(item.quantity) * Number(item.price)).toFixed(2) : '0.00'}
                      </TableCell>
                    </TableRow>
                  ))}
                  {/* Total Row */}
                  <TableRow>
                    ...
                    >
                      {s.items && s.items.reduce((sum, i) => sum + (Number(i.quantity) * (i.price || 0)), 0).toFixed(2)} AED
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
